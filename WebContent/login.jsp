...
<script type="text/javascript">
	function login(){
		...
		var isTeacher = document.getElementById("isTeacher").checked;//"true" OR "false"
		
		if(check(username,password,isTeacher)){
			//点击登陆后，禁用登陆按钮、用户名、密码输入框。知道“err”收到数据后再启用（删除此属性）
			document.getElementById("username").setAttribute("disabled", "disabled"); 
			document.getElementById("password").setAttribute("disabled", "disabled"); 
			document.getElementById("submit").setAttribute("disabled", "disabled"); 
			
			loadXMLDoc(username,password,isTeacher);
		}
	}
	function loadXMLDoc(username,password,isTeacher){
		var xmlhttp;
		...
		xmlhttp.onreadystatechange = function(){
			if(xmlhttp.readyState == 4 && xmlhttp.status == 200){
				...
				if(xmlhttp.responseText.match("s2")){ 
					xmlhttp = null;
					window.location.href = "member/teacher.jsp"; //s2老师
				}else if(xmlhttp.responseText.match("s3")){ 
					xmlhttp = null;
					window.location.href = "member/index.jsp"; //s3学生
				}
			}
		}
		xmlhttp.open("post",encodeURI("check.jsp?t=" + Math.random()+"&username="+username+"&password="+password+"&isTeacher="+isTeacher),true);
		xmlhttp.setRequestHeader("Content-type" , "application/x-www-form-urlencoded");
		xmlhttp.send();	
	}
	
	function check(username,password,isTeacher){
		if(username == ""){
        ...
        <div class="centerRight">
            ...
            <div style="height:30px"></div>
		  	<div class="tl" style="width:300px; float:right">
                ...
                <div style="width:300px; font-size: small">管理员？ <a class="xh" style="font-weight:bold;" href="adminlogin.jsp">点此</a> 去登录</div>
            </div>
        </div>
